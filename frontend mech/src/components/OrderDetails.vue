<template>
  <div class="text-center ma-4 justify-center">
    <v-row
      class="ma-4 justify-space-between align-center"
      v-for="item in incoming"
      :key="item.id"
    >
      <div class="text-left text-h4 my-2">
      ...
      <v-chip
        color="green"
        text-color="white"
        v-if="item.status == stage[3] || incoming.status == stage[4]"
        >{{ item.status }}</v-chip
      >
    </v-row>
    ...
    <v-card class="text-left mb-4 pa-4" variant="tonal">
      ...
      >
        <v-timeline-item
          ...
          :icon="items[0]"
        >
          <template v-slot:opposite> Accept </template>
        </v-timeline-item>
        ...
        <v-timeline-item
          ...
          v-model="items"
          :dot-color="
            item.status == stage[3] || item.status == stage[4]
              ? 'green'
              : 'white'
          "
          :icon="items[3]"
      ...
      <div v-for="item in incoming" :key="item.id">
        <v-card-subtitle
          v-if="item.status == stage[0]"
          class="pa-0 py-2 text-wrap"
          >{{ otw }}</v-card-subtitle
        >
        <v-card-subtitle
          v-else-if="item.status == stage[1]"
          class="pa-0 py-2 text-wrap"
          >{{ onp }}</v-card-subtitle
        >
        <v-card-subtitle
          v-else-if="item.status == stage[2]"
          class="pa-0 py-2 text-wrap"
          >{{ fin }}</v-card-subtitle
        >
      </div>
    ...
    <v-btn class="mx-1" color="error" variant="tonal" @click="dialog3 = true"
      >Cancel</v-btn
    >
    ...
    <v-dialog v-model="dialog3" width="1000" class="text-left pa-4">
      <v-card class="text-left pa-4">
        ...
        <v-card-text>
          ...
          <v-radio-group>
            <v-radio
              label="The problem was already fixed"
              value="fixed"
            ></v-radio>
    ...
    <v-dialog v-model="dialog" width="800" class="text-left pa-4">
      <v-card class="text-left pa-4">
        <v-card-title class="text-h5"
          >Status {{ incoming.status }}</v-card-title
        >
        <v-timeline
          direction="horizontal"
          line-inset="12"
        ...
        <div class="px-2">
          <v-text class="text-h6 pa-0"
            >Please type 'confirm' to continue</v-text
          >
          <v-text-field
            v-model="confirm"
            label="Confirm"
...
export default {
  ...
  methods: {
    ...
    checkStage() {
      if (
        this.ticket.status == this.stage[0] ||
        this.ticket.status == this.stage[1]
      ) {
        this.dialog = true;
      } else if (this.ticket.status == this.stage[2]) {
        this.dialog2 = true;
    ...
    mounted() {
      ...
      this.socket = new WebSocket(
        "wss://axzrwmh7sb.execute-api.us-east-1.amazonaws.com/production?ticketID=" +
    ...
    submitForm() {
      ...
      this.axios
        .post(this.$backendApi + "customer/create-profile", data)
        .then((response) => {
          console.log(response.data);
        });
    },
